...
#ifndef DISABLE_STITCHING
    if (Type == R_X86_64_PC32 && (*(BufLoc - 1) == 0xE8) && Rel.Sym->isShared() && !getOutputSection()->isSO())
    {
     
        auto symName = Rel.Sym->getName().str();
        if (Rel.Sym->isReallyIFunc() && (symName != "strcasecmp" && symName != "memcpy"))
        {
        }
        else 
        {
            auto *F = dyn_cast<SharedFile<ELF64LE>>(Rel.Sym->getFile());
            if (F != nullptr)
            {

              intptr_t absAddress = F->GetCachedSymbolAddress(symName);
              if (absAddress == 0)
              { 
                absAddress = F->GetCachedSymbolAddress("__" + symName);
            }
            if (absAddress != 0)
            {
                if (absAddress - (AddrLoc + 4) < INT32_MAX)
                {
                    TargetVA = absAddress - (AddrLoc + 4);
                }
...
switch (Expr) {
  ...
  if (BufLoc + 8 <= BufEnd && read32be(BufLoc + 4) == 0x60000000)
   ...
   // if (Type == R_X86_64_PC32)
    //    outs() << "Relocating in " << Rel.Sym->getName() << " (address " << (void*)AddrLoc << ")\n";
