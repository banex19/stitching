...
template <class ELFT>
bool SharedFile<ELFT>::CacheSymbolAddresses()
{
   std::string libName = SoName.substr(0, SoName.find('.'));

   std::string cacheInfo = "/lib/cache/" + libName + ".cache";

   auto MBOrErr = MemoryBuffer::getFile(cacheInfo);
   if (auto EC = MBOrErr.getError()) {
      return false;
  }
  ...
  std::string buffer = MBRef.getBuffer().str();
  ...
  {
    // Read address.
    intptr_t address = strtol(buffer.c_str() + curPos, nullptr, 16);

    curPos += 17;
    std::string name = buffer.substr(curPos, buffer.find('\n', curPos) - curPos);
    curPos += name.size() + 1;

   // outs() << "Address: " << (void*)address << " - " << name << "\n";
    CachedSymbolAddresses[name] = address; 
